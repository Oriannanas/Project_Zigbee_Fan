...
For ESP32-C6 SuperMini boards:
* Set target to ESP32-C6.
* Set output pin and PWM frequency in `main/light_driver.h` (`LIGHT_OUTPUT_GPIO`, `LIGHT_PWM_FREQUENCY_HZ`) if needed for your board wiring.
* If your board uses 4MB flash (common for C6 SuperMini), set `Serial flasher config -> Flash size` accordingly.
* If a selected pin is already used by your board peripherals, pick another free GPIO in `main/light_driver.h`.
