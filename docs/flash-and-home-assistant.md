...
- Current firmware PWM output pin is `GPIO9`.
...
- Install the Zigbee2MQTT external converter first — see [`docs/zigbee2mqtt-fan-converter.md`](zigbee2mqtt-fan-converter.md) — otherwise the device joins as a generic dimmable light instead of a fan.
- In Home Assistant, use the discovered fan entity (`state` + percentage `speed`).
